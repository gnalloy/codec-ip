# ADR-001: Keep codec-ip as an Independent Gnalloy Module
...
## Decision
`codec-ip` is published as `gnalloy.org/codec-ip` and owns this responsibility: IP packet codecs, protocol dispatch helpers, and checksum utilities for Gnalloy raw transports.
