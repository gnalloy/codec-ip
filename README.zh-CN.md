...
面向 Gnalloy raw transport 的 IP 包编解码器、协议分发与校验和辅助能力。

该模块位于 transport 之上、业务 handler 之下，负责把字节或 Gnalloy 消息转换成协议对象，并把出站协议对象转换回字节。它不打开 socket，也不拥有 EventLoop。

## 状态

- 导入路径：`gnalloy.org/codec-ip`
- 仓库：`github.com/gnalloy/codec-ip`
- 默认分支：`dev`
- 预览安装：`go get gnalloy.org/codec-ip@dev`
- 许可证：Apache-2.0
...
本仓库负责：面向 Gnalloy raw transport 的 IP 包编解码器、协议分发与校验和辅助能力。
